...
- 目标
  ...
  - 对“顶层装配体\汇总”驱动的“总 BOM 汇总”进行统一汇总、去重与分类，并输出 PDF。
  - 对主模型 PDF，将其下属子装配 PDF 自动合并生成完整清单。

2. 术语与缩写
- BOM：物料清单
- Toolbox：SolidWorks Toolbox 库件
- 顶层装配体\汇总：顶层装配对应的“汇总”Excel（文件名包含“汇总”）
...
- 运行环境：
  - Windows 10/11
  - Microsoft Excel 2016 或更高版本（含 VBA）
  - PDFCreator（支持 COM 自动化）
  - SolidWorks 2019 或更高版本（用于前置导出，不参与 Excel 整理环节）
...
- 输入
  ...
  - 对照表：TS180/lists.xlsm 工作簿中“ToolboxNames”工作表，B 列=英文名，C 列=中文名。
...
5.1 单个 BOM 表的整理流程
- S1 装载对照表
  - 打开 TS180/lists.xlsm，读取“ToolboxNames”工作表，建立映射字典：英文名(B) -> 中文名(C)。
...
- S4 列位置调整（整表）
  ...
  - 调整后的最终列序（从 A 到 R）应为：
    ...
    - R 标准
- S5 布尔显示图标化
  - 在 K/L/M/N（组/购/加/钣）四列中，将单元格值符合以下任一条件者（忽略大小写与空白）：是、Yes、y、j，统一替换为“○”；否则替换为“×”。
...
- S7 打印与分页设置
  ...
  - 页脚：
    - 中：第 &[页码] 页，共 &[总页数] 页
  - 页面设置：A4，横向，缩放=100%（无缩放），高质量打印，仅打印活动工作表。
- S8 PDF 导出
...
- G2 匹配与复制
  ...
  - 如找到，复制该行（按本规范最终列序或源列序，见 G5）作为候选行。
  - 若多个文件都匹配同一个 key，候选行可按“首次出现优先”保留单一行的描述性字段。
- G3 数量汇总与去重
  - 总数量 = Σ 子装配中匹配行的“数量”（F 列）之和。
  - 在新建的“总 BOM 清单”工作表中：
    - 对每个 key 仅输出一行记录（去重），其“数量”列填入“总数量”。
    - 在“备注”列的右侧新增一列“计算说明”，写入如“Σ 数量（来源：装配A、装配B、…）”或“=2+3+1=6”之类的计算说明。
- G4 不重复复制规则
  - 对相同 key（代号）仅复制一次描述性字段（名称/型号/规格等），数量使用汇总值覆盖。
- G5 列与格式
...
- G6 分类输出工作表
  - 从“总 BOM 清单”中基于 K/L/M/N（组/购/加/钣）四列生成以下分类工作表：
    - 外购件：L=“○”
    - 钣金件：N=“○”
    - 机箱模型：基于“名称”命中关键词（默认：机箱、箱体），关键词可配置。
  - 外购件工作表额外需求：
...
- G7 合并 PDF
  ...
  - 合并顺序规则：
    - 先主模型，再按 BOM 顺序（或按文件名自然排序）追加子装配 PDF。
  - 合并工具：优先使用 PDFCreator 的合并队列；如受限，可使用配置的外部合并库。

6. 配置项
- 映射表路径：TS180/lists.xlsm
- 映射表工作表名：ToolboxNames
- 关键词（机箱模型）：["机箱","箱体"]（可配置）
- 布尔真值集合：["是","Yes","y","j"]（大小写不敏感，可配置）
...
11. 验收标准与测试用例
- 用例 T1：Toolbox 名称替换
  - 输入：K 列为英文名“Hex Bolt”，映射为“内六角螺栓”。
  - 期望：E=“内六角螺栓”；L 复制到 J；M 复制到 I；其余不变。
- 用例 T2：列调整与标题重命名
...
- 用例 T7：分类表
  - 期望：外购（L=“○”）、钣金（N=“○”）、机箱模型（名称命中关键词）分别生成独立工作表并 PDF。
- 用例 T8：合并 PDF
