...
### 方法一：通过SolidWorks宏编辑器部署（从 SW_src 导入）
...
4. **导入源码文件（位于 SW_src/）**
   按下列顺序导入 SolidWorks 侧模块（复制粘贴代码内容）：
   ```
   SW_src/Constants.bas
   SW_src/Configuration.bas
   SW_src/Logger.bas
   SW_src/Utils.bas
   SW_src/RecursiveProcessor.bas
   SW_src/Main.bas
   SW_src/UpdateProperties.bas
   SW_src/UpdateProperties_UserForm.bas（含窗体）
   SW_src/frmProps.frm（窗体文件，需配合 .frx）
   ```
...
2. **运行宏**
   - SolidWorks 菜单：工具 > 宏 > 运行...
   - 选择 `ExportNestedBOM.swp`
   - 执行与你的打包版本对应的入口过程（本仓库源码的入口为 `SW_src/Main.bas` 中的导出流程与 `SW_src/UpdateProperties.bas` 的 `Run_AddCustomProps`）
...
### 行动二：在 Excel 2016 部署（BOM 表整理与 PDF）
...
3) 关闭编辑器，Excel 菜单“开发工具” → 宏 → 运行以下入口（根据需要选择）：
   - Main.Run_Merge_SubBOMs_Into_CurrentWorkbook（合并子装配表至当前工作簿）
   - Main.Run_Format_AllVisibleSheets / Main.Run_Format_CurrentSheet（格式化）
   - Main.Run_Generate_TotalBOM_FromSummary（生成“总 BOM 清单”）
   - Main.Run_Export_AllSheets_ToPDF（分表导出 PDF） / Main.Run_Print_AllSheets_Separately（分表打印）

宏安全设置（首次使用）：
- 文件 → 选项 → 信任中心 → 信任中心设置 → 宏设置 → 选“禁用所有宏并通知”或“启用所有宏（不推荐）”
- 将工程所在路径加入“受信任的位置”可避免警告提示

Excel 模块配置（位于 excel_src/Config.bas）：
- CFG_MAPPING_WORKBOOK_NAME / CFG_MAPPING_SHEET_NAME：映射表工作簿与工作表（默认 `FomatBOM_ExportPDF.xlsm` / `ToolboxNames`）
- CFG_TRUE_SET()：布尔真值集合函数（是/yes/y/j/...）
- ICON_TRUE / ICON_FALSE：布尔展示符号（默认 `●/X`）
- CFG_PDF_OutputDir：PDF 输出目录（默认 `PDF/`）
- CFG_Header_*：表头扫描/转储参数
- CFG_Enable_PDFCreator_Merge：是否启用 PDFCreator 合并（可选；检测不到 COM 时自动降级）

注意事项：
- 列标题容错：内置别名与“材 料”→“材料”的修正
- 打印区域：按 `SingleSheetFormatter` 的统一设置；标题行重复；页眉页脚按当前实现为主
- PDF 导出：默认使用 `ExportAsFixedFormat`；PDF 合并为可选（`CFG_Enable_PDFCreator_Merge`），当前仓库的 `PdfExport.TryMergePdfs` 为占位，需按本机 PDFCreator 版本自行补全
- 日志输出：运行日志以时间戳命名输出到工作簿同级 `logs/` 目录
